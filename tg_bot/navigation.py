...
def setup_navigation_handlers(bot, user_data):
    ...
    def back_to_selection(message):
        ...
        if hasattr(user_state, "consultation_mode"):
            user_state.consultation_mode = False
        if hasattr(user_state, "order_bouquet_pk"):
            del user_state.order_bouquet_pk
        if hasattr(user_state, "phone"):
