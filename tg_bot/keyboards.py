...
def create_time_keyboard():
    ...
    for i in range(0, len(times), 2):
        if i + 1 < len(times):
            markup.row(
                types.KeyboardButton(times[i]), types.KeyboardButton(times[i + 1])
            )
        else:
    ...
    if bouquets_count > 1:
        ...
        markup.row(
            types.InlineKeyboardButton(
                "⬅️ Назад", callback_data=f"bouquet_{prev_index}"
            ),
            types.InlineKeyboardButton(
                "Дальше ➡️", callback_data=f"bouquet_{next_index}"
            ),
        )

    markup.row(
        types.InlineKeyboardButton(
            "💐 Заказать этот букет", callback_data=f"order_{bouquet_pk}"
        )
    )
