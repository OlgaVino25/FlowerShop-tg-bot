...
def flower_exclusion_filter(message):
    ...
    flower_buttons = [f"❌ {flower}" for flower in flowers] + [f"✅ {flower}" for flower in flowers]
    flower_buttons.append("✅ Завершить выбор")
    return message.text in flower_buttons
...
def address_filter(message, user_data):
    ...
    return (user_id in user_data and 
            hasattr(user_data[user_id], "phone") and 
            not hasattr(user_data[user_id], "order_address"))

def date_filter(message, user_data):
    user_id = message.chat.id
    return (user_id in user_data and 
            hasattr(user_data[user_id], "order_address") and 
            not hasattr(user_data[user_id], "delivery_date"))

def time_filter(message, user_data):
    user_id = message.chat.id
    return (user_id in user_data and 
            hasattr(user_data[user_id], "delivery_date") and 
            not hasattr(user_data[user_id], "delivery_time"))

def comment_filter(message, user_data):
    user_id = message.chat.id
    return (user_id in user_data and 
            hasattr(user_data[user_id], "delivery_time") and 
            not hasattr(user_data[user_id], "comment"))